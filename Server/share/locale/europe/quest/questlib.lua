...
function pc_in_village()
	local villages = {1, 3, 21, 23, 41, 43}
	for i in villages do
		if pc.get_map_index() == villages[i] then
			return true
		end
	end
	return false
...
function npc_is_same_job()
	...
	elseif pc_job == 4 then
		if table_is_in(WOLFMAN1_NPC_LIST, npc_vnum) then return true end
		if table_is_in(WOLFMAN2_NPC_LIST, npc_vnum) then return true end
	end
	return false
end
...
function npc_get_job()
	...
	if table_is_in(WOLFMAN1_NPC_LIST, npc_vnum) then return COND_WOLFMAN_1 end
	if table_is_in(WOLFMAN2_NPC_LIST, npc_vnum) then return COND_WOLFMAN_2 end
	return 0
end
...
function get_map_name_by_number(number)
	map_name = {
		-- EmpireNr - MapNrs
		[1] = {
			[1] = gameforge.functions._100_say,
			[2] = gameforge.functions._130_say,
			[3] = gameforge.functions._130_say,
			[4] = gameforge.functions._160_say,
			[61] = gameforge.functions._200_say,
			[62] = gameforge.functions._210_say,
			[63] = gameforge.functions._220_say,
			[64] = gameforge.functions._190_say,
			[65] = gameforge.functions._230_say,
			[72] = gameforge.functions._240_say,
			[73] = gameforge.functions._240_say,
		},
		[2] = {
			[1] = gameforge.functions._110_say,
			[2] = gameforge.functions._140_say,
			[3] = gameforge.functions._140_say,
			[4] = gameforge.functions._170_say,
			[61] = gameforge.functions._200_say,
			[62] = gameforge.functions._210_say,
			[63] = gameforge.functions._220_say,
			[64] = gameforge.functions._190_say,
		...
		[3] = {
			[1] = gameforge.functions._120_say,
			[2] = gameforge.functions._150_say,
			[3] = gameforge.functions._150_say,
			[4] = gameforge.functions._180_say,
			[61] = gameforge.functions._200_say,
			[62] = gameforge.functions._210_say,
			[63] = gameforge.functions._220_say,
			[64] = gameforge.functions._190_say,
...
POINT_BLEEDING_PCT = 141
POINT_BLEEDING_REDUCE = 142
...
if file_exists(get_locale_base_path() .. "/quest/GFquestlib.lua") then
	dofile(get_locale_base_path() .. "/quest/GFquestlib.lua")
end

if file_exists(get_locale_base_path() .. "/quest/questing.lua") then
	dofile(get_locale_base_path() .. "/quest/questing.lua")
end

if file_exists(get_locale_base_path() .. "/quest/dungeonlib.lua") then
	dofile(get_locale_base_path() .. "/quest/dungeonlib.lua")
end

if file_exists(get_locale_base_path() .. "/quest/multiLocale.lua") then
	dofile(get_locale_base_path() .. "/quest/multiLocale.lua")
end
...
function get_playtime_duration(minutes)
	local minutes = tonumber(minutes) * 60
...
dofile(get_locale_base_path().."/quest/source/events_mgr_text.lua")

function time_duration(ipe) 
